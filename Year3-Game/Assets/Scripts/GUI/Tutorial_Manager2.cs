...
{
    private Ingame_GUI2 tutorialGUI;
    // Variables
    private int count = 0;
    ...
    private Tutorial_Manager TutManagerScript;
    ...
    void Start()
    {
        ...
        TutManagerScript = GameObject.FindObjectOfType<Tutorial_Manager>();
    ...
    void GiveHint()
    {
        ...
        if(Physics.Raycast(cam.transform.position, cam.transform.forward, out checkTask, 5f))
        {
            ...
            {
                if (checkTask.collider.name == "XrayTester2") //Find Partner
                {
                    HintText.enabled = true;
                   // HintText.text = "Hold X to see your partner!";
                }
                else if (checkTask.collider.name == "Cube (7)")
                {
                    HintText.enabled = true;
                    HintText.text = "Press Space to jump on red box!";
                }
                else if (checkTask.collider.name == "Cube (9)" || checkTask.collider.name == "Cube (11)")
                {
                    HintText.enabled = true;
    ...
    public void Notify(string _achievement)
    {
        ...
        {
            b_tutorialComplete = true;
            TutManagerScript.tutorialComplete += 1;
            CompleteText.enabled = true;
        }

        if (TutManagerScript.tutorialComplete == 2)
        {
