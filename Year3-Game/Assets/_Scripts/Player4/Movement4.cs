...
public class Movement4 : MonoBehaviour
{
    ...
    void Crouch()
    {
        ...
        if (controller.state4.Buttons.B == ButtonState.Pressed && controller.prevState4.Buttons.B == ButtonState.Released)
        {
            if (isCrouching == true)
            {
                isCrouching = false;
    ...
    void getJumpDown()
    {
        if (controller.state4.Buttons.A == ButtonState.Pressed)
        {
        ...
        if (controller.state4.Buttons.A == ButtonState.Released)
        {
            jumpInUse = false;
        }
    }
    ...
    void getCrouchDown()
    {
        if (controller.state4.Buttons.B == ButtonState.Pressed)
        {
        ...
        if (controller.state4.Buttons.B == ButtonState.Released)
        {
            crouchInUse = false;
        }
    }
    ...
    void FixedUpdate()
    {
        if (isGrounded())
        {
            ...
            {
                horizontalMove = controller.state4.ThumbSticks.Left.X;
                moving = true;
            }
            else
            {
            ...
            if (controller.state4.IsConnected)
            {
                verticalMove = controller.state4.ThumbSticks.Left.Y;
                moving = true;
            }
            else
            {
        ...
        bool sprint = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift) || controller.state4.Buttons.LeftStick == ButtonState.Pressed;
        ...
        else if ((Input.GetMouseButton(1) || controller.state4.Triggers.Left == 1) && isCrouching == false)
        {
            adjustedSpeed = speed /  1.10f;
        }
        else if (isCrouching == true && (Input.GetMouseButton(1) || controller.state4.Triggers.Left == 1))
        {
            adjustedSpeed = speed / 1.20f;
        }
        else if (isCrouching == true && (!Input.GetMouseButton(1) || controller.state4.Triggers.Left == 0))
        {
