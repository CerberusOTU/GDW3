...
public class Weapon4 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if ((controller.state4.Triggers.Right == 1) && !PlayerisReloading && loadout[currentIndex].ShotType == "Auto" && loadout[currentIndex].maxAmmo >= 0&& loadout[currentIndex].currentAmmo !=0)
        {
    ...
    void Update()
    {
        ...
        if (controller.state4.IsConnected)
        {
            if (grenadeAmount > 0)
            {
                ...
                //else if (Input.GetButtonUp("Grenade"))
                else if ((controller.state4.Buttons.RightShoulder == ButtonState.Released && controller.prevState4.Buttons.RightShoulder == ButtonState.Pressed))
                {
                    if (grenadeAmount == 2)
                    {
                        Grenade1.enabled = false;
        ...
        if (loadout[0] != null)
        {
            if ((controller.state4.Buttons.Y == ButtonState.Pressed && controller.prevState4.Buttons.Y == ButtonState.Released && currentIndex != 0) || (d > 0f && currentIndex != 0))
            {
                reloadCancel = true;
                Equip(0);
                //oln.enabled = true;
            ...
            else if ((controller.state4.Buttons.Y == ButtonState.Pressed && controller.prevState4.Buttons.Y == ButtonState.Released && currentIndex != 1) || (d < 0f && currentIndex != 1))
            {
                reloadCancel = true;
                Equip(1);
                oln.enabled = false;
        ...
        {
            Aim((Input.GetMouseButton(1) || controller.state4.Triggers.Left == 1));
            ...
            if (!PlayerisReloading && (Input.GetMouseButton(0) || controller.state4.Triggers.Right == 1) && currentCool <= 0 && loadout[currentIndex].ShotType == "Auto" && loadout[currentIndex].currentAmmo > 0)
            {
                origPosReset = false;
                Shoot();
            }
            // Return back to original left click position
            if ((!Input.GetMouseButton(0) || controller.state4.Triggers.Right == 0) && origPosReset == false)
            {
                ...
                //if (Mathf.Abs(cam.transform.localEulerAngles.x - saveInitShot.eulerAngles.x) <= 0.1f || Input.GetAxis("Mouse X") != 0 || Input.GetAxis("Mouse Y") != 0 || controller.state4.ThumbSticks.Right.Y != 0 || controller.state4.ThumbSticks.Right.X != 0)
                //{
        ...
        if (Input.GetMouseButton(1) || controller.state4.Triggers.Left == 1)
        {
    ...
    void Shoot()
    {
        ...
        ///-----RECOIL-----/////
        if (Input.GetMouseButtonDown(0) || controller.state4.Triggers.Right == 1 && controller.prevState4.Triggers.Right < 1)
        {
            //tempTime = Time.time;
               ...
               if (target != null)
                {
                    if (dist < 25f)
                    {
                        if (hitInfo.collider.name == "Player" || hitInfo.collider.name == "Player3")
                        {
 ...
 */
    ...
    void SwitchWeapon()
    {
        ...
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out checkWeapon, 3f))
        {
            ...
            if (checkWeapon.collider.tag == "Weapon")
            {
                ...
                if ((controller.state4.Buttons.X == ButtonState.Pressed && controller.prevState4.Buttons.X == ButtonState.Released) || Input.GetKeyDown(KeyCode.E))
                {
                    reloadCancel = true;
    ...
    void getShootUp()
    {
        if (controller.state4.Triggers.Right == 0)
        {
            if (!m_isAxisInUseUp)
            {
