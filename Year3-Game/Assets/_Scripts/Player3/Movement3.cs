...
public class Movement3 : MonoBehaviour
{
    ...
    void Crouch()
    {
        ...
        if (controller.state3.Buttons.B == ButtonState.Pressed && controller.prevState3.Buttons.B == ButtonState.Released)
        {
            if (isCrouching == true)
            {
                isCrouching = false;
    ...
    void getJumpDown()
    {
        if (controller.state3.Buttons.A == ButtonState.Pressed)
        {
        ...
        if (controller.state3.Buttons.A == ButtonState.Released)
        {
            jumpInUse = false;
        }
    }
    ...
    void getCrouchDown()
    {
        if (controller.state3.Buttons.B == ButtonState.Pressed)
        {
        ...
        if (controller.state3.Buttons.B == ButtonState.Released)
        {
            crouchInUse = false;
        }
    }
    ...
    void FixedUpdate()
    {
        if (isGrounded())
        {
            ...
            {
                horizontalMove = controller.state3.ThumbSticks.Left.X;
                moving = true;
            }
            else
            {
            ...
            if (controller.state3.IsConnected)
            {
                verticalMove = controller.state3.ThumbSticks.Left.Y;
                moving = true;
            }
            else
            {
        ...
        bool sprint = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift) || controller.state3.Buttons.LeftStick == ButtonState.Pressed;
        ...
        else if ((Input.GetMouseButton(1) || controller.state3.Triggers.Left == 1) && isCrouching == false)
        {
            adjustedSpeed = speed /  1.10f;
        }
        else if (isCrouching == true && (Input.GetMouseButton(1) || controller.state3.Triggers.Left == 1))
        {
            adjustedSpeed = speed / 1.20f;
        }
        else if (isCrouching == true && (!Input.GetMouseButton(1) || controller.state3.Triggers.Left == 0))
        {
